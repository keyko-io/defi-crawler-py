...
class Liquidations:
    ...
    def get_liquidations_from_timestamp(self, timestamp=0):
        liquidations_json = self.get_liquidations_from_api(timestamp)
        return Mappers.map_liquidation(liquidations_json, self.protocol)

    def get_liquidations_from_api(self, timestamp=0):
        are_reserve = True
        json_records = []
